...
public class StatisticsPanelController {
    ...
    public void setSimul(Simulation simul) {
        ...
        angVelLabel.textProperty().bind(Bindings.format("Angular Velocity\n%.2f m/s", this.simul.getSysAngVel()));
        aForceLabel.textProperty().bind(Bindings.format("%.2f N", this.simul.getaForce().valueProperty()));
        fForceLabel.textProperty().bind(Bindings.format("%.2f N", this.simul.getfForce().valueProperty()));
        sumForceLabel.textProperty().bind(Bindings.format("%.2f N", this.simul.getNetForce().magnitudeProperty()));

        this.simul.sysVelProperty().addListener((observable, oldValue, newValue) -> {
            velLabel.textProperty().bind(Bindings.format("Velocity\n%.2f m/s", newValue));
        });

        this.simul.sysAccProperty().addListener((observable, oldValue, newValue) -> {
            accLabel.textProperty().bind(Bindings.format("Accelerate\n%.2f m/s^2", newValue));
        });
        ...
        this.simul.objProperty().addListener((observable, oldValue, newValue) -> {
            ...
            if (newValue != null) {
                posLabel.textProperty().bind(Bindings.format("Position\n%.2f m", this.simul.getObj().positionProperty()));
...
//		aArrow.widthProperty().addListener((observable, oldValue, newValue) -> {
    ...
    private void setUpNetForce() {
        ...
        nArrow.widthProperty().addListener((observable, oldValue, newValue) -> {
            ...
            double currentSign = Math.signum(this.simul.getNetForce().getMagnitude());
