...
public class SurfacePanelController {
    ...
    public void init(Simulation simul) {
        ...
        staticCoefSlider.valueProperty().bindBidirectional(this.simul.getSurface().staticCoefficientProperty());

        // Khi hệ số ma sát tĩnh thay đổi -> Text field thay đổi
        this.simul.getSurface().staticCoefficientProperty().addListener((observable, oldValue, newValue) -> {
        ...
        staticCoefTextField.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                staticCoefTextField.setText(String.format("%.3f", this.simul.getSurface().getStaticCoefficient()));
            }
        });
        ...
        staticCoefSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
            try {
                this.simul.getSurface().setStaticCoefficient(newValue.doubleValue());
            } catch (Exception e) {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setContentText(e.getMessage() + "\nPlease input a number >= 0 and <= " + Surface.MAX_STATIC_COEFFICIENT);
                alert.getDialogPane().setMinHeight(Region.USE_PREF_SIZE);
                alert.showAndWait();
            }
        });

        kineticCoefSlider.valueProperty().bindBidirectional(this.simul.getSurface().kineticCoefficientProperty());

        this.simul.getSurface().kineticCoefficientProperty().addListener((observable, oldValue, newValue) -> {
        ...
        kineticCoefTextField.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                kineticCoefTextField.setText(String.format("%.3f", this.simul.getSurface().getKineticCoefficient()));
            }
        });

        kineticCoefSlider.valueProperty().addListener((observable, oldValue, newValue) -> {
            try {
                this.simul.getSurface().setKineticCoefficient(newValue.doubleValue());
            } catch (Exception e) {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setContentText(e.getMessage() + "\nPlease input a number >= 0 and <= " + Surface.MAX_STATIC_COEFFICIENT);
                alert.getDialogPane().setMinHeight(Region.USE_PREF_SIZE);
        ...
        this.simul.objProperty().addListener((observable, oldValue, newValue) -> {
            ((FrictionForce) this.simul.getFrictionForce()).setMainObj(newValue);
            ((FrictionForce) this.simul.getFrictionForce()).setMagnitude(0);
        });
    ...
    void staticTextFieldOnAction(ActionEvent event) {
        try {
            ...
            this.simul.getSurface().setStaticCoefficient(newValue);
    ...
    void kineticTextFieldOnAction(ActionEvent event) {
        try {
            ...
            this.simul.getSurface().setKineticCoefficient(newValue);
    ...
    public void surfaceListener() {
        try {
            this.simul.getSurface().staticCoefficientProperty().addListener((observable, oldValue, newValue) -> {
                try {
                    ((FrictionForce) this.simul.getFrictionForce()).updateFrictionForce();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
